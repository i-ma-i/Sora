...
namespace sora
{
	Graphics::Graphics(HWND hWnd)
		: m_enableWireframe(Config::GetBool("Graphics.m_enableWireframe"))
		, m_cullMode(magic_enum::enum_cast<D3D11_CULL_MODE>(Config::GetString("Graphics.m_cullMode")).value())
		, m_frontCounterClockwise(Config::GetBool("Graphics.m_frontCounterClockwise"))
	{
		// ウィンドウのサイズを取得する。
		RECT rect;
		...
		LOG_INFO("size: {}x{}", width, height);
		LOG_INFO("enableWireframe: {}", m_enableWireframe);
		LOG_INFO("cullMode: {}", magic_enum::enum_name(m_cullMode));
		LOG_INFO("frontCounterClockwise: {}", m_frontCounterClockwise);
		...
		// ラスタライザステートを作成する。
		{
			...
			rasterizerDesc.FrontCounterClockwise = m_frontCounterClockwise;
		...
		m_enableWireframe ? SetWireframeMode() : SetSolidMode();
	}
